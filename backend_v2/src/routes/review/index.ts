...
router.get('/store/:storeId', async (req, res) => {
  ...
  if (photo === 'true') query.image = { $exists: true, $type: 'string', $ne: '' };
  ...
  try {
    ...
    console.error('매장의 리뷰를 가져오던 중 에러 발생', error);
    res.sendStatus(500);
  }
});
...
router.post('/', async (req, res) => {
  const bodySchema = z.object({
    ...
    menus: z.array(z.object({ _id: z.string(), name: z.string(), like: z.coerce.boolean().optional(), dislike: z.coerce.boolean().optional() })),
    store: z.string(),
    order: z.string(),
  ...
  try {
    ...
    try {
      await Order.findByIdAndUpdate(data.order, { review: review._id });
    } catch (error) {
      console.error('주문에 리뷰 연동 중 에러 발생', error);
    }
    ...
    try {
      ...
      console.error('메뉴에 like / dislike 연동 중 에러 발생', error);
    }
