...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    console.error('매장 정보 가져오던 중 에러 발생', error);
...
router.get('/search/:query', async (req, res) => {
  ...
  try {
    ...
    if (typeof maxDeliveryPrice === 'number') aggregationPipeline.push({ $match: { deliveryPrice: { $lte: maxDeliveryPrice } } });
    if (typeof minOrderPrice === 'number') aggregationPipeline.push({ $match: { minimumOrderPrice: { $lte: minOrderPrice } } });

    aggregationPipeline.push({ $sort: SortQuery[sort] });
