...
import { storeDeliveryPriceAtom } from '@/atoms/store-filter';
import { DeliveryOption } from '@/constants/store';

import { FilterModal } from './Filter-Modal';

export const DeliveryFilterModal = ({ closeModal }: { closeModal: () => void }) => {
  ...
  return (
    <FilterModal closeModal={closeModal}>
      <div className="relative flex items-center justify-center p-3 font-bold">
        배달비
        <IoClose className="absolute right-2 text-2xl font-bold" onClick={() => closeModal()} />
      </div>
