...
import { StoreCategory, StoreCategoryImages } from '@/constants/store-category';
  ...
  return (
    <div className={`grid ${getGridCol(cols)} gap-4 p-4`}>
      {Object.values(StoreCategory).map((category) => {
        return (
          <Link className="grid place-items-stretch text-center" key={category} href={`/category/${category}`}>
            <Image className="rounded-full" src={StoreCategoryImages[category]} alt={category} width={500} height={500} />
