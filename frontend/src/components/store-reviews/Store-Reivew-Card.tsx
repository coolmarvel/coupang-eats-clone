...
export const StoreReviewCard = ({ review }: { review: Review }) => {
  ...
  const dayDiff = getDayDiff(review.createdAt ? new Date(review.createdAt) : new Date());
    ...
    <div className="border-b border-b-gray-200 pb-4">
      ...
      <div className="flex flex-wrap gap-2 pt-4">
        {review.menus.map((menu) => (
          <div
            key={menu._id}
            className={`flex items-center gap-1 rounded-full bg-blue-100 px-2 text-blue-600 ${menu.dislike ? 'bg-red-100 text-red-600' : ''}`}
          >
