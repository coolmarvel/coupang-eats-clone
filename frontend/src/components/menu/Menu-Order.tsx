...
export const MenuOrder = ({ menu }: { menu: Menu }) => {
  ...
  const handleForm: ChangeEventHandler<HTMLFormElement> = () => {
    ...
    inputs.forEach((input) => {
      ...
      const selection = additionalSelections.find((_selection) => _selection.title === selectionTitle);

      if (!selection) return;

      const option = selection.options.find((_option) => _option.title === optionTitle);

      if (!option) return;

      if (!input.checked) {
        const optionIndex = opts.findIndex((option) => option.option.title === optionTitle);
        if (optionIndex !== -1) opts = opts.slice(0, optionIndex).concat(opts.slice(optionIndex + 1));
  ...
  useEffect(() => {
    setOrderPrice(price * orderCount + options.reduce((sum, option) => sum + option.option.price, 0));
  ...
  return (
    <>
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between">
            ...
            <div className="flex items-center gap-2">
              <CiCircleMinus className="text-4xl text-gray-400" onClick={decrementCount} />
              <p className="text-lg">{orderCount}</p>
              <CiCirclePlus className="text-4xl text-gray-400" onClick={incrementCount} />
            </div>
          </div>
        </div>
        <form ref={ref} onChange={handleForm}>
          {additionalSelections?.map((selection) => (
            <div key={selection.title}>
              <h3 id={selection.title} className="flex items-center justify-between bg-gray-100 p-4 font-bold">
                <div>{selection.title}</div>
                {selection.required && <div className="text-sm font-normal text-orange-600">필수 선택</div>}
              </h3>
              <fieldset>
                {selection.options.map((option) => (
                  <div key={option.title} className="flex items-center gap-2 p-4">
                    <input
                    ...
                    <label htmlFor={`${selection.title}-${option.title}`}>
                      {option.title}
                      {option.price && <span className="text-gray-400">(+{KRW(option.price)})</span>}
                    </label>
                  </div>
                ))}
              </fieldset>
      ...
      <button className="fixed bottom-0 flex h-20 w-screen items-center justify-center bg-blue-500 pb-4 text-lg text-white" onClick={addToCart}>
        배달 카트에 담기
      </button>
    </>
  );
