...
export default function OrderReview({ orderId }: { orderId: string }) {
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-12 gap-2">
          ...
          <div className="col-span-10 grid gap-4 pr-4">
            ...
            <div className="flex gap-2">
              <input type="file" id="imageUpload" name="imageUpload" onChange={handleFileUpload} className="hidden" />
              <label htmlFor="imageUpload" className="flex flex-col items-center justify-center border border-gray-400 p-2 text-sm">
                <BsCamera className="text-3xl" />
                사진 추가
              </label>
              {image && <img src={image} alt="uploaded image" className="h-16 w-16" />}
            </div>
            {menus.map(({ _id, name, like, dislike }) => (
              <div className="flex" key={_id}>
                <div className="flex-grow">{name}</div>
                <div className="flex gap-2 text-gray-500">
                  <button className={`rounded-full border px-3 py-2 ${like ? 'bg-blue-100 text-blue-500' : ''}`} onClick={() => handleLikeMenu(_id)}>
                    <FaRegThumbsUp />
                  </button>
                  <button
      ...
      <button className="fixed bottom-0 grid h-16 w-screen place-items-center" onClick={handleSubmitReview}>
        등록하기
      </button>
    </>
  );
