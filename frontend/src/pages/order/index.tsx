...
export default function Order() {
      ...
      <div className="flex h-screen flex-col">
        ...
        <div className="grid flex-grow place-items-center">
          <div className="grid place-items-center gap-2 text-gray-500">
            ...
            장바구니가 비어있습니다
            <Link href="/" className="rounded-lg border border-black p-2 text-black">
              쿠팡이츠 맛집 구경가기
            </Link>
          </div>
        </div>
  ...
  const totalPrice = cart.menus.reduce((acc, menu) => acc + getMenuPrice(menu), 0);
    ...
    <>
      <div className="grid gap-2  pb-20">
        ...
        <div className="grid gap-2 border-t-8 border-t-gray-200 bg-white">
          <Link className="border-b border-b-gray-100 p-4 text-lg font-bold" href={`/store/${storeId}`}>
            {store.name}
          </Link>
          {cart.menus?.map((menu) => (
            <div key={menu.menu._id} className="grid gap-2 border-b border-b-gray-100 p-4">
              ...
              {menu.options?.map((option) => (
                <div key={option.option.title} className="text-sm text-gray-500">
          ...
          <Link href={`/store/${storeId}`} className="grid place-items-center p-2 text-blue-500">
            + 메뉴추가
          </Link>
        </div>
      </div>
